		...
		if (!this.material.getItem(this.tag)) {
			...
			for (let i = 0; i < Math.min(icon.length, 6); i++) {
				...
				for (let part of config.parts) {
					let paint = new Paint();
					if (part.paint) paint.setColorFilter(new ColorFilter(Color.parseColor(this.material.color), PorterDuff.Mode.MULTIPLY));
					texture.draw(`${TextureSource.getPath(this.material.key) + part.name}.png`, { paint: paint });
				}
